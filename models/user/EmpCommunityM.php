...
class EmpCommunityM extends CI_Model
{
	...
	{
		return $this->db->select("c.*,cat.CategoryName,ct.CityName,s.StateName,e.UserName,e.CoverImage")->from("tblcommunityemp ce")->join("tblcommunity c","c.CommunityId=ce.CommunityId")->join("tblcategory cat","cat.CategoryId=c.CategoryId")->join("tblcity ct","ct.CityId=c.CityId")->join("tblstate s","s.StateId=ct.StateId")->join("tblemployee e","e.EmployeeId=c.EmployeeId")->where_in("ce.EmployeeId",$data)->get()->result();
	}

	public function selectCommunityByEmployeeId($data)
	{
		return $this->db->select("c.*,cat.CategoryName,ct.CityName,s.StateName,e.UserName,e.CoverImage")->from("tblcommunity c")->join("tblcategory cat","cat.CategoryId=c.CategoryId")->join("tblcity ct","ct.CityId=c.CityId")->join("tblstate s","s.StateId=ct.StateId")->join("tblemployee e","e.EmployeeId=c.EmployeeId")->where_in("c.EmployeeId",$data)->get()->result();
	}
	...
	{
		return $this->db->select("ccr.*,u.Username,u.ProfileImage,u.MobileNo,u.Email,c.CoverPic,c.Title")->from("tblcommunityrequest ccr")->join("tbluser u","u.UserId=ccr.UserId")->join("tblcommunity c","c.CommunityId=ccr.CommunityId")->where_in("c.CommunityId",$data)->get()->result();
	}
	...
	public function insertCommunityEmp($data)
	{
		return $this->db->insert("tblcommunityemp",$data);
	}
